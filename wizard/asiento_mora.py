...
class MoratorioAsiento(models.Model):
    ...
    def run(self):
        for res in self:
            ...
            # move = self.env["account.move.line"].create(apunte_credit)
            # move1 = self.env["account.move.line"].create(apunte_debit)
            # move.post()
            # move1.post()