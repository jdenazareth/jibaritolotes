...
class AccountFollowupReport(models.AbstractModel):
    ...
    def _get_columns_name(self, options):
        ...
        if self.env.company.ji_apply_developments:
            if len(headers) > 0:
                headers.insert(1, {'name': _('Correlative')})
        return headers

    def _get_lines_jibarito(self, options, line_id):
        ...
        line_num = 0
        for l in partner.unreconciled_aml_ids.filtered(lambda l: l.company_id == self.env.company).sorted(
                lambda l: l.ji_sequence_payments):
    ...
    def _get_lines(self, options, line_id=None):
        if self.env.company.ji_apply_developments:
            return self._get_lines_jibarito(options, line_id)
        return super(AccountFollowupReport, self)._get_lines(options, line_id)
